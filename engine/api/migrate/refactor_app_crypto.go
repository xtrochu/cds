...
func refactorApplicationCrypto(ctx context.Context, db *gorp.DbMap, id int64) error {
	...
	app, err := application.LoadByIDWithClearVCSStrategyPassword(ctx, tx, id)
	if err != nil {
	...
	app, err = application.LoadByID(ctx, tx, id, application.LoadOptions.WithClearDeploymentStrategies)
	if err != nil {
		return err
	}
